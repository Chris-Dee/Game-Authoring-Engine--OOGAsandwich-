 ...
 * Gifs, JPegs, or PNGs (PNGs not in java 1.2), which may contain either single
 * images, or regularly spaced arrays or matrices of images (i.e. sprite sheets,
 * we will call them image maps). An image is identified by a unique name, and
 * is defined by the image file it comes from, the index within that file in
 * case the file is an image map, and any flip and rotate actions that are to be
 * be done on the file image to arrive at the final image. Animations may be
 ...
 * keycodes. MIDP does not support mouse yet, so the mouse state is always
 * coordinate (0,0) and no mouse buttons pressed.
 * 
 * <p>
 ...
 * on and off for the entire application. Sound is not yet implemented in MIDP,
 * so sound calls are ignored.
 * 
 * <p>
...
public abstract class JGEngine extends Applet implements JGEngineInterface {
	...
	public void defineImage(String name, String tilename, int collisionid,
			String imgfile, String img_op, int top, int left, int width,
			int height) {
		el.defineImage(this, name, tilename, collisionid, imgfile, img_op, top,
				left, width, height);
	}
	...
	public void defineImage(String imgname, String tilename, int collisionid,
			String imgmap, int mapidx, String img_op, int top, int left,
	...
	public void defineImageMap(String mapname, String imgfile, int xofs,
			int yofs, int tilex, int tiley, int skipx, int skipy) {
	...
	void drawObject(Graphics g, JGObject o) {
		...
		if ((debugflags & JGEngine.BBOX_DEBUG) != 0) {
			...
			if (bbox != null) { // tile bounding box defined
				...
				bbox.height *= el.tiley;
				bbox = el.scalePos(bbox, true);
				g.drawRect(bbox.x + 2, bbox.y + 2, bbox.width - 4,
						bbox.height - 4);
			}
		}
		// o.frameFinished();
	...
	/**
	 * xi,yi are tile indexes relative to the tileofs, that is, the top left of
	 * the bg, + 1. They must be within both the tilemap and the view.
	 */
	public void drawTile(int xi, int yi, int tileid) {
		...
		if (img == null || el.images_transp.containsKey(tileid_obj)) {
			EngineLogic.BGImage bg_image = (EngineLogic.BGImage) el.bg_images
					.get(0);
			if (bg_image == null) {
				...
				bgg.drawImage(((JREImage) el.getImage(bg_image.imgname)).img,
						...
						(xtile + 1) * el.scaledtilex, (ytile + 1)
								* el.scaledtiley, (Color) el.bg_color.impl,
						null);
			}
		}
		if (img != null)
	...
	void copyBGToBuf(Graphics bufg, int sx1, int sy1, int sx2, int sy2,
		...
		int dx2 = dx1 + sx2 - sx1;
		int dy2 = dy1 + sy2 - sy1;
		bufg.drawImage(background, dx1 * el.scaledtilex - barreldx, dy1
				* el.scaledtiley - barreldy, dx2 * el.scaledtilex - barreldx,
				dy2 * el.scaledtiley - barreldy,
	...
	class JGCanvas extends Canvas {
		...
		public void paint(Graphics g) {
			try {
				...
				/*
				 * Each frame before the paint operation, we check if our
				 * possibly volatile bg and buffer are still ok. If so, we don't
				 * re-validate every time, but leave them as persistent images
				 * until their contents are eventually destroyed. Then we always
				 * recreate them even if their status is RESTORED. Note that bg
				 * is indeed persistent, and things are incrementally drawn on
				 * it during the course of doFrames and paints. If the buffer to
				 * be rendered to screen is invalid when we render it to screen,
				 * we give up for this frame and don't retry until the next
				 * frame.
				 */
				if (background == null
						|| !JREImage.isScratchImageValid(background)) {
					background = JREImage.createScratchImage(el.width + 3
							* el.scaledtilex, el.height + 3 * el.scaledtiley);
				...
				if (buffer != null && background != null) {
					...
					synchronized (el.objects) {
						...
						int tilexshift = el.moduloFloor(el.tilexofs + 1,
								el.viewnrtilesx + 3);
						int tileyshift = el.moduloFloor(el.tileyofs + 1,
								el.viewnrtilesy + 3);
	...
	void paintDbgFrameLogs(Graphics g) {
		...
		Font dbgfont = new Font(debugmessage_font.name,
				debugmessage_font.style, (int) debugmessage_font.size);
		g.setFont(dbgfont);
		for (Enumeration e = dbgframelogs.keys(); e.hasMoreElements();) {
			...
			if (obj == null) {
				...
				if (obj != null) {
					...
					if (deadtime < dbgframelog_expiry) {
						dbgframelogs_dead
								.put(source, new Integer(deadtime + 1));
					} else {
			...
			if (obj != null) {
				JGPoint scaled = el.scalePos(obj.x - el.xofs, obj.y - el.yofs
						+ lineheight / 3, false);
	...
	public void dbgIsEmbedded(boolean enabled) {
		isEmbedded = enabled;
	}
	...
	/**
	 * Construct engine, but do not initialise it yet. Call initEngine or
	 * initEngineApplet to initialise the engine.
	 */
	public JGEngine() {
	...
	public void setCanvasSettings(int nrtilesx, int nrtilesy, int tilex,
			int tiley, JGColor fgcolor, JGColor bgcolor, JGFont msgfont) {
	...
	public void setScalingPreferences(double min_aspect_ratio,
			double max_aspect_ratio, int crop_top, int crop_left,
	...
	/**
	 * Initialise engine; don't call directly. This is supposed to be called by
	 * the applet viewer or the initer.
	 */
	public void init() {
		if (el.winwidth == 0) {
		...
		canvas.setInitPainter(new ListCellRenderer() {
			public Component getListCellRendererComponent(JList d1,
				...
				setFont(g, el.msg_font);
				setColor(g, el.fg_color);
				JGImage splash = el.existsImage("splash_image") ? el
						.getImage("splash_image") : null;
				if (splash != null) {
					JGPoint splash_size = getImageSize("splash_image");
					drawImage(g, viewWidth() / 2 - splash_size.x / 2,
							viewHeight() / 4 - splash_size.y / 2,
							"splash_image", false);
				}
				drawString(g, canvas.progress_message, viewWidth() / 2,
						viewHeight() / 2, 0, false);
				...
				drawRect(
						g,
						(int) (viewWidth() * (0.1 + 0.8 * canvas.progress_bar)),
						(int) (viewHeight() * 0.6),
				...
				// length stripes
				drawRect(g, (int) (viewWidth() * 0.1),
						(int) (viewHeight() * 0.6), (int) (viewWidth() * 0.8),
				...
				drawString(g, canvas.author_message, viewWidth() - 16,
						viewHeight() - getFontHeight(g, el.msg_font) - 10, 1,
						false);
				return null;
			}
		});
	 ...
	 * It is also the default colour for painting
	 */
	...
	/**
	 * Set the (unscaled) message font, used for displaying status messages. It
	 * is also the default font for painting.
	 */
	...
	/**
	 * Set foreground and background colour, and message font in one go; passing
	 * a null means ignore that argument.
	 */
	...
	private BufferedImage null_image = new BufferedImage(1, 1,
	...
	void paintFrame(Graphics g) {
		...
		if ((debugflags & GAMESTATE_DEBUG) != 0) {
			...
			setFont(el.msg_font);
			setColor(g, el.fg_color);
			drawString(state, el.viewWidth(), el.viewHeight()
					- (int) getFontHeight(g, el.msg_font), 1);
		}
		if ((debugflags & MSGSINPF_DEBUG) != 0)
	...
	public void drawLine(double x1, double y1, double x2, double y2,
		...
		// y2 -= canvas.yofs;
		// }
		buf_gfx.drawLine(el.scaleXPos(x1, pf_relative),
				el.scaleYPos(y1, pf_relative), el.scaleXPos(x2, pf_relative),
	...
	public void drawImage(double x, double y, String imgname,
			JGColor blend_col, double alpha, double rot, double scale,
			boolean pf_relative) {
		if (buf_gfx == null)
			return;
		drawImage(buf_gfx, x, y, imgname, pf_relative);
	...
	public void drawImageString(String string, double x, double y, int align,
		...
		el.drawImageString(this, string, x, y, align, imgmap, char_offset,
				spacing, false);
	}
	...
	void paintExitMessage(Graphics g) {
		try {
			...
			setColor(g, debug_auxcolor2);
			// draw colour bars
			drawRect(g, el.viewWidth() / 2, el.viewHeight() / 2 - 5 * height
					/ 2, 9 * viewWidth() / 10, 5, true, true, false);
			drawRect(g, el.viewWidth() / 2, el.viewHeight() / 2 + 5 * height
					/ 2, 9 * viewWidth() / 10, 5, true, true, false);
			setColor(g, el.fg_color);
			...
			while (toker.hasMoreTokens()) {
				drawString(g, toker.nextToken(), el.viewWidth() / 2, ypos, 0,
						false);
				ypos += height + 1;
			}
